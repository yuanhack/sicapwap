...
CWStateTransition CWWTPEnterConfigure() {
	...
	if(!CWErr(CWWTPSendAcknowledgedPacket(seqNum,
		...
		gWTPSession = NULL;
#endif
		CWLog(" **************** CWWTPEnterConfigure return CW_QUIT");
		return CW_QUIT;
	}
	
	CWLog(" **************** CWWTPEnterConfigure return CW_ENTER_DATA_CHECK");
...
CWBool CWParseConfigureResponseMessage (char *msg,
    ...
    while((completeMsg.offset-offsetTillMessages) < controlVal.msgElemsLen) {
        ...
        // CWLog("Configure Message Element type %d[%02x], len %d[%02x]", type, type, len, len);
        switch(type) {
            ...
            case CW_MSG_ELEMENT_VENDOR_SPEC_PAYLOAD_CW_TYPE:  // Yuan Hong
                ...
                if (sino_count > 0) { 
                    sino_count--;
                    CWParseVendorPayloadSinoix(&completeMsg, len);
                }
                sino_unlock();
                break;
#endif

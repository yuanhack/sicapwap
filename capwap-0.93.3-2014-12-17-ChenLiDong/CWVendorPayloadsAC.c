	...
	switch (valuesPtr->vendorPayloadType){
			case CW_MSG_ELEMENT_VENDOR_SPEC_PAYLOAD_WUM:
				...
				case WTP_CANCEL_UPDATE_REQUEST:
					CWLog("[PHP]: Entry WTP_VERSION_REQUEST.");
				...
				case WTP_UPDATE_REQUEST:
					CWLog("[PHP]: Entry WTP_UPDATE_REQUEST.");
				...
				case WTP_CUP_FRAGMENT:
					CWLog("[PHP]: Entry WTP_CUP_FRAGMENT.");
				...
				CWLog("[PHP]: msgType = %d wumPr->type = %d.", msgType, wumPtr->type);

...
CWBool ACEnterRun(int WTPIndex, CWProtocolMessage *msgPtr, CWBool dataFlag) {
	...
	if(dataFlag){
		...
		CWDebugLog("debug by lidong msgPtr->data_msgType = %d.", msgPtr->data_msgType);
		...
		CWDebugLog("debug by lidong [ACEnterRun] StationMacAddr[%02x:%02x:%02x:%02x:%02x:%02x]", \
								StationMacAddr[0],
								StationMacAddr[1],
								StationMacAddr[2],
								StationMacAddr[3],
		...
		//Send a Station Configuration Request
			 ...
			 ************************************************************/
			fprintf(stderr, "\n debug by lidong Manage special data packets with frequency!\n");
		  ...
		  if(msgPtr->data_msgType == CW_DATA_MSG_STATS_TYPE)
			{			  
			  if(!UnixSocksArray[WTPIndex].data_stats_sock)
				...
				printf("\ndebug by lidong %s\t%s",socketserv_path_name,socketctl_path_name);fflush(stdout);
				}
			  ...
			  fprintf(stderr, "\n debug by lidong sendto over\n");
			  if (nbytes < 0) 
				{
				  fprintf(stderr, "\n debug by lidong sendto nbytes < 0\n");
	...
	while((msgPtr->offset - offsetTillMessages) < len) {
		...
		switch(elemType) {
			...
			case CW_MSG_ELEMENT_RESULT_CODE_CW_TYPE_WITH_PAYLOAD:
				...
				*resultCode=CWProtocolRetrieve32(msgPtr);
				CWDebugLog("[PHP] quqing CW_MSG_ELEMENT_RESULT_CODE_CW_TYPE_WITH_PAYLOAD resultCode = %d.", *resultCode);
	...
	while((msgPtr->offset - offsetTillMessages) < len) {
		...
		switch(elemType) {
			case CW_MSG_ELEMENT_CW_DECRYPT_ER_REPORT_CW_TYPE:
				...
				CWDebugLog("debug by lidong o<>o o^o o-o o_o for check CW_MSG_ELEMENT_CW_DECRYPT_ER_REPORT_CW_TYPE.");
...
CWBool CWParseChangeStateEventRequestMessage2(CWProtocolMessage *msgPtr,
	...
	while((msgPtr->offset-offsetTillMessages) < len) {
		...
		CWDebugLog("#__uncomment__# Parsing Message Element: %u, elemLen: %u", elemType, elemLen);

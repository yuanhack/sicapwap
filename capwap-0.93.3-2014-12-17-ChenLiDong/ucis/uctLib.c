...
#define UCI_INIT_REQ_MSG(msg, size) do { msg.payload_len = 0; msg.payload=malloc(size); } while(0);
#define UCI_DESTROY_MSG(msg)  do { if (msg.payload_len != 0) free(msg.payload); } while(0);
...
void UCIPayloadStore8(uci_req_t *req, char c);
void WUMPayloadStore32(wum_req_t *req, int i);
void UCIPayloadStore32(uci_req_t *req, int i);
void WUMPayloadStoreRawBytes(wum_req_t *req, void *buf, int size);
void UCIPayloadStoreRawBytes(uci_req_t *req, void *buf, int size);

int UCISendCmdRequest(char cmd, int acserver, int wtpId, void *buf, int size);
...
int WUMReadCupVersion(char *cup_pathname, struct version_info *update_v)
{
	...
	snprintf(buf, BUF_SIZE, "tar xzf %s -C /tmp", cup_pathname);
...
int UCIModifyCmd(char cmd, int acserver, int wtpId, void *buf, int size)
{
	if (UCISendCmdRequest(cmd, acserver, wtpId, buf, size)) {
		fprintf(stderr, "Set Config Request failed for WTP: %d\n", wtpId);
		return ERROR;
	}
...
int UCISendCmdRequest(char cmd, int acserver, int wtpId, void *buf, int size)
{
	...
	UCI_INIT_REQ_MSG(msg, size);	
	msg.cmd_msg = CONF_UPDATE_MSG;
	msg.msg_elem = MSG_ELEMENT_TYPE_VENDOR_UCI;
	msg.wtpId = wtpId;
	msg.uci_command = cmd;

	fprintf(stderr, "..........msg....payload_len = %d\n", msg.payload_len);

	//UCIPayloadStore32(&msg, gTest);
	UCIPayloadStoreRawBytes(&msg, buf, size);
	fprintf(stderr, "..........msg....payload_len = %d\n", msg.payload_len);

	if (UCISendMessage(acserver, msg) != 0) {
		fprintf(stderr, "Error while sending WUM message");
		return ERROR;
	}
	fprintf(stderr, "..........msg....payload_len = %d\n", msg.payload_len);

	if (UCIReceiveMessage(acserver, &resp) != 0) {
		fprintf(stderr, "Error while response message");
		return ERROR;
	}

	/* debug */
	fprintf(stderr, "......response resp.wtpid = %d\n", resp.wtpId);
	if (!resp.resultCode)
		fprintf(stderr, "......response resp.resultCode = CW_PROTOCOL_SUCCESS\n");
	else if (resp.resultCode == CW_PROTOCOL_FAILURE)
		fprintf(stderr, "......response resp.resultCode = CW_PROTOCOL_FAILURE\n");
	else
		fprintf(stderr, "......response resp.resultCode = %d\n", resp.resultCode);

	fprintf(stderr, "......response resp.payload_len = %d\n", resp.payload_len);
	if (resp.payload_len) {
		fprintf(stderr, "......response resp.payload = %s\n", resp.payload);
		fprintf(stderr, "......response resp.payload = %02x%02x%02x%02x%02x%02x\n", \
								(unsigned char)resp.payload[0],
								(unsigned char)resp.payload[1],
								(unsigned char)resp.payload[2],
								(unsigned char)resp.payload[3],
								(unsigned char)resp.payload[4],
								(unsigned char)resp.payload[5]
								);
	}
...
int UCISendMessage(int acserver, uci_req_t msg)
{
	/* Fix byte order issues*/
	msg.wtpId = htonl(msg.wtpId);
	msg.payload_len = htonl(msg.payload_len);

	fprintf(stderr, "[UCISendMessage]: acserver = %d .", acserver);

	if (Writen(acserver, &msg, 11) != 11) {
		fprintf(stderr, "Error while sending ---- message.\n");
                return ERROR;
	}
	fprintf(stderr, "[UCISendMessage]: write 11 msg over!\n");
...
int UCIReceiveMessage(int acserver, uci_resp_t *msg)
{
	int len;

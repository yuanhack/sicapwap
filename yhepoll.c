...
fd_event * fd_event_new()
{
    ...
    if (fe == 0) {
        CWLog("fd_event_new() calloc error[%d]", errno);
        return 0;
    }
...
{
    if (fe == 0) return;
    //if (fe->ptr) { free(fe->ptr); fe->ptr = 0; } // user del
    if (fe->heap == FD_EVENT_MAGIC_HEAP) free(fe);
}
...
int em_fd_event_add(fd_event* fe)
{
    ...
    if (ret < 0) CWLog("em_fd_event_add() epoll_ctl %d fd %d error[%d]"
...
int em_fd_event_mod(fd_event* fe)
{
    ...
    if (ret < 0) CWLog("em_fd_event_mod() epoll_ctl %d fd %d error[%d]"
            , fe->em->epfd, fe->fd, errno);
    return ret;
}
int em_fd_event_del(fd_event* fe)
{
    ...
    if (ret < 0) CWLog("em_fd_event_del() epoll_ctl %d fd %d error[%d]"
            , fe->em->epfd, fe->fd, errno);
    return ret;
}
...
int setfd_nonblock(int fd)
{
    ...
    if ((status = fcntl(fd, F_GETFL)) < 0) { 
        CWLog("setfd_nonblock() fcntl F_GETFL error[%d]", errno); 
        return -1; 
    }
    status |= O_NONBLOCK;
    if (fcntl(fd, F_SETFL, status) < 0) { 
        CWLog("setfd_nonblock() fcntl F_SETFL error[%d]", errno); 
        return -1; 
    }
    return 0;
}
int setsock_rcvtimeo(int fd, int second, int microsecond)
{
    ...
    if (setsockopt(fd,SOL_SOCKET,SO_RCVTIMEO,&rcv_timeo,sizeof(rcv_timeo))< 0) {
        CWLog("setsock_rcvtimeo() setsockopt SO_RCVTIMEO error[%d]", errno);
        return -1; 
    }
    return 0;
}
        ...
        em_callback before, em_callback events, em_callback after)
{
    ...
    { CWLog("Em_open() em_open error[%d]", errno); exit(1); }
    return em;
}
...
int em_run(epoll_manager *em)
{
    ...
    if ((ret = pthread_create(&em->tid, &attr, em_thread, em)) != 0) {
        errno = ret;
        CWLog("em_start() pthread_create error[%d]", errno);
        return -1;
    }
...
int em_fd_event_notify(fd_event *fe, char ch, int flag)
{
    ...
    if (ret < 0) 
        CWLog("em_fd_event_notify() writev [%d] [0x%08lx:%d %c %d] error[%d]"
                , fe->em->cfd[1], (long)fe, fe->fd,ch,flag,errno); /*
    else
        CWLog("em_fd_event_notify() writev [%d] [0x%08lx:%d %c %d] success"
                , fe->em->cfd[1], (long)fe, fe->fd, ch, flag); // */
    return ret;
...
int em_notify(epoll_manager *em, char ch, int flag)
{
    ...
    if (ret < 0) CWLog("em_notify() writev [%d] [%c] error[%d]",
            em->cfd[1], ch, errno);
    return ret;
}
int em_stop(epoll_manager *em)
{
    ...
    if (em->run > 0) {
        if ((ret = em_notify(em, 'S', 0)) < 0) { 
            CWLog("em_stop() em_notify error[%d]", errno);
            pthread_mutex_unlock(&em->lock); 
            return ret;
        }
    }
...
int em_close(epoll_manager *em)
{
    ...
    if (ret < 0)
        CWLog("em_close() em_nitofy error[%d]", errno);
    return ret;
}
...
#define EPOLL_EVENT_READV_ERR_EXIT()  \
    do { \
        CWLog("em_event_process() readv error[%d]", errno); \
        exit(1);\
    } while (0)
